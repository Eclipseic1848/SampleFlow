...
- API 标准输出为单行 JSON。请求日志包含时间、级别、服务、请求标识、方法、路由模板、状态、耗时、操作、结果和固定原因码；不记录请求体、查询参数、账号、客户、订单或导入内容。
- API 容器内的 `/internal/metrics` 输出 Prometheus 文本格式，覆盖 HTTP 请求量、错误量、耗时、登录／审批／导入失败和数据库就绪状态。标签仅限路由模板、方法、状态类别、操作、结果和固定原因码。
- `/internal/metrics` 不由 Web 代理，采集器只能从受控内部网络直连 API。公司监控平台接入、告警阈值和通知链仍需在服务器环境确认。
