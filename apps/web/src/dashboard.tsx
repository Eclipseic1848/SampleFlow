...
export function Dashboard({ user, onLogout, onChangePassword }: { user: User; onLogout: () => void; onChangePassword:()=>void }) {
  ...
  const navigate=useCallback((page:PageId,mode:"push"|"replace"="push",query:Record<string,string>={})=>{if(mode==="push"&&readPageId(window.location.search)===page)return;const params=new URLSearchParams(window.location.search);params.set("page",page);for(const [key,value] of Object.entries(query))params.set(key,value);window.history[`${mode}State`]({},"",`${window.location.pathname}?${params.toString()}${window.location.hash}`);focusPageHeading.current=mode==="push";setActive(page);},[]);
