...
export function OrdersPage({ user }: { user: User }) {
  ...
  return <main className="dashboard orders-page" data-onboarding-page="orders"><header data-onboarding="page-header"><div><h1>订单业绩</h1><p>按订单编号维护不可变业绩事件；已入账记录不能覆盖或删除</p></div>{canExport||canEdit ? <div className="header-actions" data-onboarding="page-actions">{canExport?<button className="secondary-action" disabled={exporting} onClick={exportOrders}>{exporting?"正在导出…":"导出当前授权范围匹配订单"}</button>:null}{canEdit?<><button className="secondary-action" onClick={() => setShowImport(true)}><FileUp size={16}/>Excel 导入</button><button className="primary-action" onClick={() => setShowCreate(true)}><Plus size={16}/>录入新订单</button></>:null}</div> : null}</header>
    ...
    <section className="orders-card" data-onboarding="primary-content" aria-labelledby="orders-table-title"><div className="orders-toolbar"><div><h2 id="orders-table-title">订单台账</h2><span role="status">{loading?"正在查询…":`本页 ${orders.length} 笔订单`}</span></div><button className="icon-action" onClick={() => refresh()} aria-label="刷新订单"><RefreshCw size={17}/></button></div>
      ...
      <div className="orders-table-wrap"><table><thead><tr><th scope="col">收样月份</th><th scope="col">日期</th><th scope="col">订单编号</th><th scope="col">客户姓名</th><th scope="col">客户单位</th><th scope="col">省份</th><th scope="col">业务员</th><th scope="col">部门</th><th scope="col">组别</th><th scope="col">系统营业额</th><th scope="col">服务类型</th><th scope="col">备注</th><th scope="col">协作人 / 比例</th><th scope="col">计入业绩</th><th scope="col">状态</th><th scope="col">操作</th></tr></thead><tbody>{!loading&&orders.length === 0 ? <tr><td colSpan={16} className="empty-cell">{emptyMessage}</td></tr> : orders.map((order) => <tr key={order.id}><td>{Number(order.sourceReceivedOn.slice(5,7))}月</td><td>{order.sourceReceivedOn}</td><td>{order.orderNo}</td><td>{order.customerName}</td><td>{order.customerUnit}</td><td>{order.businessRegionSourceText??businessRegionName(order.businessRegionCode??"")}</td><td>{order.salespersonName}</td><td>{order.departmentName||"—"}</td><td>{order.groupName||"—"}</td><td>{order.lifecycleState==="receivable_pending"?<span className="negative">应收未收 {formatMoney(Math.abs(Number(order.currentRevenue)))}</span>:formatMoney(order.currentRevenue)}</td><td>{order.serviceType||"—"}</td><td>{order.note||"—"}</td><td>{order.collaboratorName?`${order.collaboratorName} / ${Number(order.collaborationRatio)*100}%`:"—"}</td><td>{formatMoney(order.countedAmount)}</td><td><Status state={order.lifecycleState}/></td><td><button className="table-action" onClick={() => setSelected(order)}>查看 / 调整</button></td></tr>)}</tbody></table></div>
