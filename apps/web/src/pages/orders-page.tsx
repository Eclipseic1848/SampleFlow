...
import { Field, Modal, PaginatedCollection, Pagination, Status, parsePageNumber, parsePageSize, type PageSize } from "../shared-ui";
...
export function OrdersPage({ user }: { user: User }) {
  ...
  return <main className="dashboard orders-page" data-onboarding-page="orders"><header data-onboarding="page-header"><div><h1>订单业绩</h1><p>按订单编号维护不可变业绩事件；已入账记录不能覆盖或删除</p></div>{canExport||canEdit ? <div className="header-actions" data-onboarding="page-actions">{canExport?<button className="secondary-action" disabled={exporting} onClick={exportOrders}>{exporting?"正在导出…":"导出当前授权范围匹配订单"}</button>:null}{canEdit?<><button className="secondary-action" onClick={() => setShowImport(true)}><FileUp size={16}/>Excel 导入</button><button className="primary-action" onClick={() => setShowCreate(true)}><Plus size={16}/>录入新订单</button></>:null}</div> : null}</header>
    ...
    <section className="orders-card" data-onboarding="primary-content" aria-labelledby="orders-table-title"><div className="orders-toolbar"><div><h2 id="orders-table-title">订单台账</h2><span role="status">{loading?"正在查询…":`本页 ${orders.length} 笔订单`}</span></div><button className="icon-action" onClick={() => refresh()} aria-label="刷新订单"><RefreshCw size={17}/></button></div>
      ...
      <div className="orders-table-wrap order-ledger-scroll"><table className="order-ledger-table" aria-label="订单台账"><colgroup>{Array.from({length:16},(_,index)=><col key={index}/>)}</colgroup><thead><tr><th className="order-key-month" scope="col">收样月份</th><th className="order-key-date" scope="col">日期</th><th className="order-key-number" scope="col">订单编号</th><th scope="col">客户姓名</th><th scope="col">客户单位</th><th scope="col">省份</th><th scope="col">业务员</th><th scope="col">部门</th><th scope="col">组别</th><th scope="col">系统营业额</th><th scope="col">服务类型</th><th scope="col">备注</th><th scope="col">协作人 / 比例</th><th className="order-key-counted" scope="col">计入业绩</th><th className="order-key-status" scope="col">状态</th><th className="order-key-actions" scope="col">操作</th></tr></thead><tbody>{!loading&&orders.length === 0 ? <tr><td colSpan={16} className="empty-cell">{emptyMessage}</td></tr> : orders.map((order) => <tr key={order.id}><td className="order-key-month">{Number(order.sourceReceivedOn.slice(5,7))}月</td><td className="order-key-date">{order.sourceReceivedOn}</td><td className="order-key-number" title={order.orderNo}>{order.orderNo}</td><td title={order.customerName}>{order.customerName}</td><td title={order.customerUnit}>{order.customerUnit}</td><td>{order.businessRegionSourceText||businessRegionName(order.businessRegionCode??"")||<span className="data-pending" title="历史订单尚未完成受控分析维度补齐">待补齐</span>}</td><td title={order.salespersonName}>{order.salespersonName}</td><td title={order.departmentName||undefined}>{order.departmentName||"—"}</td><td title={order.groupName||undefined}>{order.groupName||"—"}</td><td>{order.lifecycleState==="receivable_pending"?<span className="negative">应收未收 {formatMoney(Math.abs(Number(order.currentRevenue)))}</span>:formatMoney(order.currentRevenue)}</td><td title={order.serviceType||undefined}>{order.serviceType||"—"}</td><td title={order.note||undefined}>{order.note||"—"}</td><td title={order.collaboratorName||undefined}>{order.collaboratorName?`${order.collaboratorName} / ${Number(order.collaborationRatio)*100}%`:"—"}</td><td className="order-key-counted">{formatMoney(order.countedAmount)}</td><td className="order-key-status"><Status state={order.lifecycleState}/></td><td className="order-key-actions"><button className="table-action" onClick={() => setSelected(order)}>查看 / 调整</button></td></tr>)}</tbody></table></div>
...
function CreateOrder({ onClose, onSaved }: { onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <Modal title="录入订单业绩" note="字段与标准 Sheet3 模板一致；组织归属按日期自动核验" onClose={onClose} preventClose={saving}><form className="business-form" onSubmit={submit}>
    ...
    {error?<p className="form-error" role="alert">{error}</p>:null}<div className="modal-actions"><button type="button" onClick={onClose} disabled={saving}>取消</button><button type="submit" disabled={saving} aria-busy={saving}>{saving?"正在入账…":"确认入账"}</button></div>
  </form></Modal>;
}
