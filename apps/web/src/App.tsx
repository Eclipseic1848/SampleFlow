...
function ChangePassword({onChanged}:{onChanged:()=>Promise<void>}){const[currentPassword,setCurrentPassword]=useState("");const[newPassword,setNewPassword]=useState("");const[message,setMessage]=useState("");async function submit(event:FormEvent){event.preventDefault();const response=await fetch("/api/auth/change-password",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({currentPassword,newPassword})});const data=await response.json() as {message?:string};if(!response.ok){setMessage(data.message??"密码修改失败");return;}await onChanged();}return <main className="password-shell"><section className="login-card"><div className="login-heading"><p>首次登录安全设置</p><h2>请修改初始密码</h2><span>新密码至少 12 位，修改后其他会话将失效</span></div><form onSubmit={submit}><label>当前密码</label><input type="password" value={currentPassword} onChange={(e)=>setCurrentPassword(e.target.value)}/><label>新密码</label><input type="password" value={newPassword} onChange={(e)=>setNewPassword(e.target.value)}/><button type="submit">保存新密码</button>{message?<p className="form-error">{message}</p>:null}</form></section></main>}
...
function AccountsPage({user}:{user:User}){
  const [users,setUsers]=useState<AdminUser[]>([]);const [roles,setRoles]=useState<Array<{code:string;name:string}>>([]);const [message,setMessage]=useState("");const [showCreate,setShowCreate]=useState(false);const isAdmin=user.roles.includes("system_admin");
  ...
  async function toggle(item:AdminUser){const response=await fetch(`/api/admin/users/${item.id}/status`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({isActive:!item.isActive})});const data=await response.json() as {message?:string};if(!response.ok){setMessage(data.message??"状态修改失败");return;}await load();}
  return <main className="dashboard"><header><div><h1>账号管理</h1><p>系统管理权限与业务权限分离；业务角色必须显式分配</p></div>{isAdmin?<button className="primary-action" onClick={()=>setShowCreate(true)}><Plus size={16}/>创建账号</button>:null}</header>{!isAdmin?<div className="permission-note"><ShieldCheck size={18}/>仅独立系统管理员可以维护账号和角色。</div>:null}{message?<p className="page-message">{message}</p>:null}{isAdmin?<section className="orders-card"><div className="orders-toolbar"><div><h2>系统账号</h2><span>{users.length} 个账号</span></div></div><div className="orders-table-wrap"><table><thead><tr><th>账号</th><th>姓名</th><th>角色</th><th>状态</th><th>操作</th></tr></thead><tbody>{users.map((item)=><tr key={item.id}><td>{item.username}</td><td>{item.displayName}</td><td>{item.roles.map((role)=>roleNames[role]??role).join("、")}</td><td><span className={`status ${item.isActive?"status-active":"status-paused"}`}>{item.isActive?"启用":"停用"}</span></td><td><button className="table-action" onClick={()=>toggle(item)}>{item.isActive?"停用":"启用"}</button></td></tr>)}</tbody></table></div></section>:null}{showCreate?<CreateAccount roles={roles} onClose={()=>setShowCreate(false)} onSaved={async()=>{setShowCreate(false);await load();}}/>:null}</main>;
}

function CreateAccount({roles,onClose,onSaved}:{roles:Array<{code:string;name:string}>;onClose:()=>void;onSaved:()=>Promise<void>}){const[username,setUsername]=useState("");const[displayName,setDisplayName]=useState("");const[password,setPassword]=useState("SampleFlow@2026");const[role,setRole]=useState(roles[0]?.code??"salesperson");const[error,setError]=useState("");async function submit(event:FormEvent){event.preventDefault();const response=await fetch("/api/admin/users",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({username,displayName,password,roles:[role]})});const data=await response.json() as {message?:string};if(!response.ok){setError(data.message??"创建失败");return;}await onSaved();}return <Modal title="创建系统账号" note="新账号首次登录后应修改初始密码" onClose={onClose}><form className="business-form" onSubmit={submit}><div className="form-grid"><Field label="登录账号" value={username} onChange={setUsername}/><Field label="显示姓名" value={displayName} onChange={setDisplayName}/><Field label="初始密码" value={password} onChange={setPassword}/><label className="field"><span>角色</span><select value={role} onChange={(event)=>setRole(event.target.value)}>{roles.map((item)=><option key={item.code} value={item.code}>{item.name}</option>)}</select></label></div>{error?<p className="form-error">{error}</p>:null}<div className="modal-actions"><button type="button" onClick={onClose}>取消</button><button type="submit">创建账号</button></div></form></Modal>}
...
function GoalsPage({ user, pendingOnly }: { user: User; pendingOnly: boolean }) {
  ...
  async function action(goal: Goal, kind: "sign"|"approve"|"reject"|"request") {
    ...
    const response=await fetch(url,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)});const data=await response.json() as {message?:string};if(!response.ok){setMessage(data.message??"操作失败");return;}await load();
  }
  const canExport=user.roles.some((role)=>role==="hr"||role==="general_manager");
...
function CreateOrder({ onClose, onSaved }: { onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/performance/orders", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...form, amount: Number(form.amount) }) });
      const data = await response.json() as { message?: string };
...
function AdjustOrder({ order, canEdit, onClose, onSaved }: { order: Order; canEdit: boolean; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  async function submit(event: FormEvent) {
    ...
    const response = await fetch(`/api/performance/orders/${order.id}/events`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
    const data = await response.json() as { message?: string };
