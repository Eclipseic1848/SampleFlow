...
test("关闭期间仅允许经人事批准的单次范围更正且普通跨月调整进入操作月",async()=>{
  await withMigratedTestDatabase(async(database)=>{
    ...
    await withTestApi(database.url,async(app)=>{
      ...
      const requested=await app.inject({method:"POST",url:"/api/accounting-corrections",headers:leader,payload:{periodMonth:"2026-08",orderId:Number(correctionOrderId),eventType:"revenue_change",occurredOn:"2026-08-20",reason:"八月原金额核对有误"}});
      ...
      const client=new Client({connectionString:database.url});await client.connect();
      try{
        const evidence=await client.query<{normal_month:string;correction_month:string;correction_occurred_at:string;correction_events:string;request_status:string;audits:string;closure_versions:string;needs_reclose:boolean}>(
          `select
  ...
  await withMigratedTestDatabase(async(database)=>{
    ...
    await withTestApi(database.url,async(app)=>{
      ...
      const request=async(orderId:string,reason:string)=>{
        const response=await app.inject({method:"POST",url:"/api/accounting-corrections",headers:leader,payload:{periodMonth:"2026-08",orderId:Number(orderId),eventType:"revenue_change",occurredOn:"2026-08-20",reason}});
        assert.equal(response.statusCode,201,response.body);return response.json().id as string;
      };
...
test("历史待核订单保留原始事件语义并经组长核对和人事批准后解锁",async()=>{
  await withMigratedTestDatabase(async(database)=>{
    ...
    try{
      const order=await client.query<{id:string}>(
        `insert into performance_orders
          (qingflow_order_no,customer_name,customer_unit,salesperson_person_id,salesperson_name,source_received_on,
           original_amount,current_revenue,counted_amount,lifecycle_state,posted_at)
         values('LEGACY-REVIEW','历史待核客户','测试单位',$1,'账本业务员','2026-08-01',0,0,-50,'historical_review_required',now())
      ...
      await client.query(
        `insert into performance_events
          (order_id,event_type,delta_amount,resulting_current_revenue,resulting_counted_amount,accounting_month,occurred_on,
           ...
           salesperson_person_id,department_unit_id,group_unit_id,leader_person_id,supervisor_person_id)
         values($1,'legacy_adjustment',50,0,50,'2026-08-01','2026-08-01','原始正向行','账本业务员','账本部门','账本小组','账本组长','账本主管',90001,$2,$3,$4,$5,$6),
               ($1,'legacy_adjustment',-100,0,-50,'2026-08-01','2026-08-02','原始负向行','账本业务员','账本部门','账本小组','账本组长','账本主管',90002,$2,$3,$4,$5,$6)`,common,
      );
    }finally{await client.end();}
    ...
    await withTestApi(database.url,async(app)=>{
      ...
      const verify=new Client({connectionString:database.url});await verify.connect();
      try{
        const evidence=await verify.query<{legacy_total:string;all_total:string;state:string;current_revenue:string;review_status:string}>(
          `select
             ...
             (select status from historical_order_reviews where id=$2) as review_status`,[orderId,reviewId],
        );
