...
export const ROLE_POLICIES = {
  ...
  sales_assistant_leader: { name: "销售助理组长", performanceScope: "all", goalScope: "none", businessOperations: ["全公司业绩录入与调整", "导入人员与统计归属补齐", "月度核对与关闭月更正", "历史订单核对"], targetResponsibilities: "无", exportPermission: "全公司业绩", forbidden: ["目标编辑与审批", "账号与正式组织授权维护"], performanceEdit: true, accountAdmin: false, organizationAdmin: false },
  ...
  general_manager: { name: "总经理", performanceScope: "all", goalScope: "all", businessOperations: ["全公司业务查看", "下达销售经理总目标"], targetResponsibilities: "下达销售经理总目标；审批销售经理自行发起的总目标", exportPermission: "全公司业务", forbidden: ["业绩录入与调整", "直接改写目标版本", "账号与组织维护"], performanceEdit: false, accountAdmin: false, organizationAdmin: false },
...
export function pendingGoalSql(goalAlias: string, versionAlias: string, firstParameter: number): string {
  return `(($${firstParameter+3}::boolean and ${versionAlias}.status in ('pending_signature','pending_gm','pending_hr')) or (${versionAlias}.status='pending_signature' and ${goalAlias}.owner_person_id=$${firstParameter}
      and (${goalAlias}.goal_level='sales_manager' or ${versionAlias}.created_by_person_id<>$${firstParameter}))
    or ($${firstParameter + 1}::boolean and ${versionAlias}.status='pending_gm' and not ${versionAlias}.gm_issued and ${goalAlias}.goal_level='sales_manager'
...
export function pendingGoalValues(user: CurrentUser): unknown[] {
  return [user.personId, user.roles.includes("general_manager"), user.roles.includes("hr"), isAcceptanceOperator(user)];
}
